...
void print_usage(char const* prog)
{
    std::cerr
        ...
        << "                                        minimal = ~4.25 GiB floor, fits 5 GiB+\n"
        << "                                                  (4 GiB cards still don't\n"
        << "                                                   fit — needs CUB sort + Xs\n"
        << "                                                   tiling cuts not yet ported)\n"
...
bool parse_devices_arg(std::string const& s, pos2gpu::BatchOptions& opts)
{
    ...
    //   "cpu"      → opts.include_cpu     = true (pos2-chip CPU plotter)
