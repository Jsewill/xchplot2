...
// SpillTileReader — the disk-tile callback the spill path passes in. Defined in
// its own SYCL-free header so SpillEngine.hpp can produce one without pulling
// <sycl/sycl.hpp> into a host-only translation unit. See that header for the
// slot discipline each variant below relies on.
