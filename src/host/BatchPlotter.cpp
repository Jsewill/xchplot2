                      ...
                      BatchOptions const& opts)
{
    ...
    // selected", fall back to the default SYCL selector, and either plot on an
    ...
    // should fall through to the GPU (or to the selector's own no-device error on
    // a GPU-less host), exactly as a plain GPU run always has.
