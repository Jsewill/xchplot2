// Cancel.hpp — SIGINT/SIGTERM/SIGHUP handling for long-running batches.
//
...
//
// SIGHUP is included so SSH disconnect doesn't kill the process
// mid-CUDA-kernel — the cancel flag lets the batch drain cleanly.
//
...
// Install SIGINT + SIGTERM + SIGHUP handlers. Idempotent — safe to
// call more than once. First signal sets the cancel flag and prints
// a one-line notice via write(2) (async-signal-safe). Second signal
// of the same type re-raises with the default disposition,
