...
streaming tier for that device. Tier ∈ `plain|compact|minimal|tiny|pinned|auto`.
...
| `XCHPLOT2_SPILL_IO_THREADS=N` | Worker threads the spill engine splits each disk job across, 1-32. Default 2. Each job is cut into one byte range per thread, so this costs no extra staging memory. On the NVMe it was tuned against, 1 → 2 was worth ~16% of the spill stall and 2 → 4 was not — the drive stops rewarding concurrency at about two outstanding operations. A RAID or a datacentre drive may have a deeper sweet spot; measure before raising it. `1` restores the original serial behaviour. |
...
- **Compact streaming (~5.2 GB peak + 128 MB margin; ≥ 5.20 GiB free
  ...
  **5200 MB** (measured on sm_89); per-phase live maxes:
  ...
  Note that the trace only shows allocations routed through the
  pipeline's accounting. It is not the true device peak, and it should
  not be used on its own to calibrate a tier: a raw `sycl::malloc_device`
  is invisible to it. That is not hypothetical — the T2/T3 two-phase
  match candidate scratch was one, ~3.1 GB of it, which put every
  non-tiny tier ~3 GB over its advertised peak and made 4-11 GB NVIDIA
