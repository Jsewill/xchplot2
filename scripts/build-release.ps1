...
# Build a Windows SYCL/NVIDIA archive with the LLVM-integrated AdaptiveCpp toolchain.
...
$env:PATH = "$prefix/bin;$env:CUDA_PATH/bin;" + $env:PATH
...
python "$prefix/bin/acpp" "--acpp-deploy=core,cuda:$runtime"
...
cmake -S . -B $BuildDir -G Ninja -DCMAKE_BUILD_TYPE=Release `
    ...
    '-DACPP_TARGETS=generic;omp' -DXCHPLOT2_BUILD_CUDA=ON -DXCHPLOT2_PACKAGE_GPU=nvidia `
